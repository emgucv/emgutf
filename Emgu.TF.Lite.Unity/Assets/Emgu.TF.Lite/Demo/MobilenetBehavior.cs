...
namespace Emgu.TF.Lite
{
    public class MobilenetBehavior : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                _displayMessage = String.Format("Downloading Inception model files, {0} % of file {1}...",
        ...
        private void RecognizeAndUpdateText(Texture texture)
        {
            ...
            if (results != null)
            {
                resStr = String.Format(
                    "Object is {0} with {1}% probability. Recognition completed in {2} milliseconds.", results[0].Label,
                    results[0].Probability * 100, watch.ElapsedMilliseconds);
            }
